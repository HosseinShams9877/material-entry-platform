...
async function main() {
  ...
  for (const u of usersData) {
    const user = await db.user.upsert({
      ...
      update: { role: u.role, workshopId: u.workshopId, fullName: u.fullName },
      create: { username: u.username, passwordHash: await hashPassword('123456'), fullName: u.fullName, role: u.role, workshopId: u.workshopId },
    })
  ...
  console.log('✅ Seed complete. حساب‌های آزمایشی: admin / manager.sa / supervisor.sa / supervisor.da / warehouse.sa / inspector.sa / viewer.sa — گذرواژه همه: 123456')
}
