...
import { ClipboardCheck, Clock3, CheckCircle2, ArrowLeftRight, HandCoins, Building2, ChevronLeft, FileBarChart, Timer, AlarmClock, ListTodo, CheckCheck, FileText, FileSignature, ShoppingCart, ClipboardList, Package, Wallet, AlertTriangle, TrendingUp } from 'lucide-react'
...
import { ENTRY_TYPES, ROLES } from '@/lib/permissions'
import { formatRelative, formatQty, toFa, formatJalaliFromISO } from '@/lib/fa'
import { Button } from '@/components/ui/button'
import { ERROR_MSG } from '@/components/app/messages'

/** نمایش فشردهٔ مبلغ تومانی — «۱۲٫۵ م» برای داشبورد */
function faMoneyCompact(amount: string | null | undefined): string {
  if (!amount) return '—'
  const n = Number(amount)
  if (!Number.isFinite(n)) return '—'
  if (n >= 1_000_000_000) return `${toFa(Math.round((n / 1_000_000_000) * 10) / 10)} میلیارد`
  if (n >= 1_000_000) return `${toFa(Math.round((n / 1_000_000) * 10) / 10)} م`
  if (n >= 1_000) return `${toFa(Math.round(n / 1_000))} هـ`
  return toFa(n)
}
    ...
    <div className="max-w-lg mx-auto">
      <div className="safe-top bg-zinc-900 text-white px-5 pt-6 pb-14 rounded-b-3xl -mb-10">
        ...
        <p className="text-xs text-zinc-400 mt-1">
          {ROLES[session?.user.role as keyof typeof ROLES] ?? 'مدیر'} — {session?.scope?.isGlobal ? 'همهٔ پروژه‌ها' : session?.projects[0]?.name ?? 'همه پروژه‌ها'}
        </p>
      </div>
      ...
      {/* خرید و انبار و گزارش کار — سند سیستم یکپارچه */}
      {data && (data.purchases.pending > 0 || data.purchases.approved > 0 || data.purchases.ordered > 0 || data.inventory.totalStocks > 0 || data.workReportsToday > 0) ? (
        <div className="px-4 mb-5">
          <div className="grid grid-cols-3 gap-2">
            <button
              type="button"
              onClick={() => navigate('purchase-requests')}
              className="rounded-xl border border-border bg-card p-3 text-right active:scale-[0.98] transition-transform"
            >
              <ShoppingCart className={"size-4 mb-1 " + (data.purchases.stale > 0 ? 'text-red-600' : 'text-accent')} />
              <p className="text-base font-bold numeric-input">{toFa(data.purchases.pending)}</p>
              <p className="text-[10px] text-muted-foreground leading-4">
                درخواست در انتظار
                {data.purchases.stale > 0 ? ` — ${toFa(data.purchases.stale)} بی‌پاسخ` : ''}
              </p>
            </button>
            <button
              type="button"
              ...
              className="rounded-xl border border-border bg-card p-3 text-right active:scale-[0.98] transition-transform"
            >
              <Package className={"size-4 mb-1 " + (data.inventory.lowStock > 0 ? 'text-red-600' : 'text-accent')} />
              <p className="text-base font-bold numeric-input">{toFa(data.inventory.lowStock)}</p>
              <p className="text-[10px] text-muted-foreground leading-4">
                کمبود انبار
                {data.inventory.totalStocks > 0 ? ` از ${toFa(data.inventory.totalStocks)} قلم` : ''}
              </p>
            </button>
            <button
              type="button"
              ...
              className="rounded-xl border border-border bg-card p-3 text-right active:scale-[0.98] transition-transform"
            >
              <ClipboardList className="size-4 mb-1 text-accent" />
              <p className="text-base font-bold numeric-input">{toFa(data.workReportsToday)}</p>
              <p className="text-[10px] text-muted-foreground leading-4">گزارش کار امروز</p>
            </button>
          </div>
        </div>
      ) : null}

      {/* مالی ساده — پرداخت/سررسید/بدهی (سند سیستم یکپارچه) */}
      {data?.finance ? (
        <div className="px-4 mb-5">
          <div className="grid grid-cols-3 gap-2">
            <button
              ...
              className="rounded-xl border border-border bg-card p-3 text-right active:scale-[0.98] transition-transform"
            >
              <Wallet className="size-4 mb-1 text-accent" />
              <p className="text-base font-bold numeric-input">{faMoneyCompact(data.finance.debtTotal)}</p>
              <p className="text-[10px] text-muted-foreground leading-4">بدهی تأمین‌کنندگان</p>
            </button>
            <button
              type="button"
              onClick={() => navigate('finance')}
              className={"rounded-xl border p-3 text-right active:scale-[0.98] transition-transform " + (data.finance.overdueCount > 0 ? 'border-red-200 bg-red-50/50' : 'border-border bg-card')}
            >
              <AlertTriangle className={"size-4 mb-1 " + (data.finance.overdueCount > 0 ? 'text-red-600' : 'text-muted-foreground')} />
              <p className={"text-base font-bold numeric-input " + (data.finance.overdueCount > 0 ? 'text-red-600' : '')}>{toFa(data.finance.overdueCount)}</p>
              <p className="text-[10px] text-muted-foreground leading-4">سررسید گذشته</p>
            </button>
            <button
              type="button"
              onClick={() => navigate('finance')}
              className="rounded-xl border border-border bg-card p-3 text-right active:scale-[0.98] transition-transform"
            >
              <TrendingUp className="size-4 mb-1 text-amber-500" />
              <p className="text-base font-bold numeric-input">{faMoneyCompact(data.finance.receivablesOpenAmount)}</p>
              <p className="text-[10px] text-muted-foreground leading-4">طلب باز از کارفرما</p>
            </button>
          </div>
        </div>
      ) : null}
          ...
          <div className="px-4 pb-8">
            <div className="grid grid-cols-3 gap-2.5 mb-3">
              <button
                ...
                className="rounded-2xl border border-border bg-card p-3.5 text-right active:scale-[0.98] transition-transform"
              >
                <FileSignature className="size-5 text-accent mb-2" />
                <span className="block text-xs font-medium">صورت وضعیت‌ها</span>
                <span className="block text-[10px] text-muted-foreground mt-0.5">تأیید و امضا</span>
              </button>
              <button
                type="button"
                onClick={() => navigate('purchase-requests')}
                className="rounded-2xl border border-border bg-card p-3.5 text-right active:scale-[0.98] transition-transform"
              >
                <ShoppingCart className="size-5 text-accent mb-2" />
                <span className="block text-xs font-medium">درخواست‌های خرید</span>
                <span className="block text-[10px] text-muted-foreground mt-0.5">اعلام نیاز انبار</span>
              </button>
              <button
                type="button"
                onClick={() => navigate('work-reports')}
                className="rounded-2xl border border-border bg-card p-3.5 text-right active:scale-[0.98] transition-transform"
              >
                <ClipboardList className="size-5 text-accent mb-2" />
                <span className="block text-xs font-medium">گزارش کار</span>
                <span className="block text-[10px] text-muted-foreground mt-0.5">کارِ کارگران</span>
              </button>
            </div>
