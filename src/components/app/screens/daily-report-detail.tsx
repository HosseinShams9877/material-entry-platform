...
import { AudioPlayer } from '@/components/app/daily-shared'
import { formatJalaliDateTime, formatJalaliFromISO, formatRelative } from '@/lib/fa'
      ...
      <div className="p-4 space-y-4">
        <div className="rounded-2xl border border-border bg-card p-4">
          ...
          <div className="grid grid-cols-2 gap-2 mt-3 text-[11px] text-muted-foreground">
            ...
            <span className="flex items-center gap-1">
              ...
              تاریخ: {formatJalaliFromISO(new Date(report.reportDate).toISOString().slice(0, 10))}
            </span>
            <span>پروژه: {report.projectName}</span>
            <span>کارگاه: {report.workshopName}</span>
            {report.sourceType === 'VOICE' ? <span className="text-accent">گزارش صوتی</span> : null}
          </div>
        </div>
        ...
        {report.audio ? (
          <div className="space-y-2">
            <AudioPlayer audioUrl={report.audio.audioUrl} fileName={`گزارش صوتی — ${report.audio.fileName}`} />
            {report.audio.transcribeStatus === 'FAILED' ? (
        ...
        {report.transcript ? (
          <div className="rounded-xl border border-border bg-secondary/60 p-3">
            <p className="text-[11px] text-muted-foreground mb-1">متن خام استخراج‌شده از صوت:</p>
            <p className="text-xs leading-6 whitespace-pre-wrap">{report.transcript}</p>
          </div>
        ) : null}
