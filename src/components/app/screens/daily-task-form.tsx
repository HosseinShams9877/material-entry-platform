...
import { Plus, Trash2, ArrowUp, ArrowDown, Save, Send, Sparkles, Loader2, CalendarDays, AlarmClock } from 'lucide-react'
import { toast } from 'sonner'
import { Button } from '@/components/ui/button'
import { Input } from '@/components/ui/input'
import { Textarea } from '@/components/ui/textarea'
import { api, ClientApiError, type AudioUploadResult, type DailyTaskDetail, type ExtractedItem, type MasterDataWithSupervisors } from '@/lib/client'
...
import { toFa, todayISO, formatJalaliFromISO } from '@/lib/fa'
...
export default function DailyTaskForm({ params }: { params?: Record<string, unknown> }) {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then(({ task }) => {
        ...
        setAssignedDate(new Date(task.assignedDate).toISOString().slice(0, 10))
        if (task.dueDate) {
        ...
        setTranscript(task.sourceTranscript ?? '')
        if (task.audio) setAudioId(task.audio.id)
        setLoaded(true)
      })
      .catch(() => {
  ...
  async function extractItems() {
    ...
    if (text.length < 5) {
      toast.error('ابتدا صوت ثبت کنید یا متن دستور را بنویسید.')
      return
    }
    setExtracting(true)
    try {
      const res = await api.post<{ items: ExtractedItem[] }>('/api/v1/daily-tasks/extract-items', { transcript: text })
      setItems((prev) => {
        const base = prev.filter((p) => p.id !== null || p.title.trim().length > 0)
        const fromAi = res.items.map((it, idx) => ({
          key: `ai-${Date.now()}-${idx}`,
      ...
      toast.success(`${toFa(res.items.length)} آیتم پیشنهادی افزوده شد — قابل ویرایش است.`)
    } catch (err) {
  ...
  function buildPayload() {
    return {
      ...
      sourceTranscript: transcript.trim() || null,
    }
  }
  ...
  return (
    <div className="max-w-lg mx-auto pb-8">
      ...
      <div className="p-4 space-y-4">
        ...
        {!editId ? (
          <div className="space-y-2">
            <AudioRecorder onUploaded={onAudioUploaded} label="دستور صوتی (اختیاری)" />
            <AudioHint transcribeStatus={transcript ? 'DONE' : audioId ? 'FAILED' : null} />
            {(transcript.trim().length > 0 || audioId) && (
              <Textarea
                value={transcript}
                onChange={(e) => setTranscript(e.target.value)}
                rows={3}
                className="text-sm leading-7 bg-card"
                placeholder="متن دستور — پس از تبدیل صوت قابل ویرایش است"
              />
            )}
            <Button type="button" variant="outline" className="w-full h-11" onClick={extractItems} disabled={extracting}>
              {extracting ? <Loader2 className="size-4 animate-spin" /> : <Sparkles className="size-4" />}
              تبدیل متن به آیتم‌های اجرایی
            </Button>
          </div>
        ) : null}
        ...
        <div className="rounded-xl border border-border bg-card p-3">
          ...
          {items.length === 0 ? (
            <p className="text-xs text-muted-foreground pb-2">هیچ آیتمی اضافه نشده است — از دکمهٔ پایین یا «تبدیل متن» استفاده کنید.</p>
          ) : (
