...
import {
  ...
  CheckCheck, User, Clock, AudioLines, ChevronDown, ChevronUp, Camera,
} from 'lucide-react'
...
import { AudioPlayer, TaskProgress } from '@/components/app/daily-shared'
      ...
      <div className="p-4 space-y-4">
        ...
        <div className={cn('rounded-2xl border p-4', task.isOverdue ? 'border-red-200 bg-red-50/50' : 'border-border bg-card')}>
          ...
          <div className="grid grid-cols-2 gap-2 mt-3 text-[11px] text-muted-foreground">
            ...
            <span className="flex items-center gap-1">
              ...
              {formatJalaliFromISO(new Date(task.assignedDate).toISOString().slice(0, 10))}
            </span>
            {task.dueDate ? (
        ...
        {task.audio ? (
          <div className="space-y-2">
            <AudioPlayer audioUrl={task.audio.audioUrl} fileName={`دستور صوتی — ${task.audio.fileName}`} />
            {task.audio.transcribeStatus === 'FAILED' ? (
              <p className="text-[11px] text-orange-600">تبدیل صوت به متن ناموفق بوده است.</p>
            ) : null}
          </div>
        ) : null}
        {task.sourceTranscript ? (
          <div className="rounded-xl border border-border bg-card p-3">
            <button type="button" onClick={() => setShowTranscript((v) => !v)} className="w-full flex items-center justify-between text-sm font-medium">
            ...
            {showTranscript ? <p className="text-sm leading-7 mt-2 whitespace-pre-wrap">{task.sourceTranscript}</p> : null}
          </div>
        ) : null}
        ...
        <div className="space-y-2">
          ...
          {task.comments.map((c) => (
            <div key={c.id} className="rounded-xl border border-border bg-card p-3">
              ...
              {c.audio ? (
                <div className="mt-2">
                  <AudioPlayer audioUrl={c.audio.audioUrl} fileName="پیام صوتی" />
                </div>
              ) : null}
            </div>
          ))}
