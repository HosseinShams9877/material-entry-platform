...
import VoiceEntry from '@/components/app/screens/voice-entry'
import VoicePreview from '@/components/app/screens/voice-preview'
...
const NO_NAV_SCREENS = [
  'login', 'voice-entry', 'voice-preview', 'manual-entry', 'entry-detail',
...
function ScreenRenderer({ name, params }: { name: string; params?: Record<string, unknown> }) {
  switch (name) {
    ...
    case 'voice-entry':
      return <VoiceEntry />
    case 'voice-preview':
      return <VoicePreview params={params} />
    case 'manual-entry':
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((me) => {
        ...
        if (role === 'PROJECT_MANAGER') resetTo('manager-dashboard')
