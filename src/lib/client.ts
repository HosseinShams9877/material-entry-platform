...
export interface MeData {
  user: { id: string; username: string; fullName: string; role: string; workshopId: string | null; phone: string | null; workerId: string | null }
  permissions: string[]
...
export interface DashboardManager {
  ...
  transfers: number
  loans: number
  purchases: { pending: number; approved: number; ordered: number; stale: number }
  inventory: { lowStock: number; totalStocks: number }
  workReportsToday: number
  /** مالی ساده — فقط برای کاربران با مجوز finance.view */
  finance: {
    debtTotal: string
    overdueCount: number
    overdueTotal: string
...
export interface StatementListItem {
  ...
  title: string
  status: string
  amount: string // تومان — رشته برای پرهیز از خطای عددی
  needsGmSign: boolean
  workshopId: string
  workshopName: string
...
export interface PurchaseRequestDetail {
  ...
  /** مالی ساده — سررسید و پرداخت‌ها (سند سیستم یکپارچه) */
  dueDate: string | null
...
export interface WorkerTransferItem {
  id: string
  workerId: string
  workerName: string
