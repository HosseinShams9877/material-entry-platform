...
/** رخدادهای ماژول صوت (حذف‌شده) — از فهرست حسابرسی پنهان می‌شوند */
const LEGACY_VOICE_ACTIONS = [
  'VOICE_PROCESS',
  'TRANSCRIBE_AUDIO',
  'TRANSCRIPTION_FAILED',
  'UPLOAD_TASK_AUDIO',
  'UPLOAD_REPORT_AUDIO',
  'UPLOAD_COMMENT_AUDIO',
]
...
export const GET = apiHandler(async ({ req, user }) => {
  ...
  let where: Record<string, unknown> = { action: { notIn: LEGACY_VOICE_ACTIONS } }
  if (!globalUser) {
    if (user.role === 'WORKSHOP_MANAGER') {
      ...
      where = {
        AND: [
          { action: { notIn: LEGACY_VOICE_ACTIONS } },
          {
            OR: [
              { userId: user.id },
      ...
      where = { AND: [{ action: { notIn: LEGACY_VOICE_ACTIONS } }, { userId: user.id }] }
    }
  }
  if (action && action !== 'ALL') where = { AND: [where as Prisma.AuditLogWhereInput, { action }] }
