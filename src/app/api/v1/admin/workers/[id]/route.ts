    ...
    async ({ body, user, params, ip, userAgent }) => {
      ...
      const updated = await db.worker.update({
        ...
        data: {
          ...
          jobTitle: body.jobTitle !== undefined ? body.jobTitle : target.jobTitle,
      ...
      await writeAudit({ user, action: 'EDIT', entityType: 'Worker', entityId: id, oldValue: { fullName: target.fullName }, newValue: { fullName: updated.fullName, isActive: updated.isActive, jobTitle: updated.jobTitle }, ip, userAgent })
