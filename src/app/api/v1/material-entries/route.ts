  ...
  async ({ body, user, ip, userAgent }) => {
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      try {
        ...
        // اتصال Transcript صوتی در صورت وجود (Audit مسیر Voice)
      if (input.voiceTranscriptId) {
        await db.voiceTranscript
          .updateMany({ where: { id: input.voiceTranscriptId, createdById: user.id }, data: { entryId: entry.id } })
          .catch(() => undefined)
      }
      ...
      await writeAudit({
        ...
        entityId: entry.id,
        newValue: { entryNumber: entry.entryNumber, type: entry.type, status: entry.status, source: input.voiceTranscriptId ? 'VOICE' : 'MANUAL' },
        ip,
        userAgent,
      })
