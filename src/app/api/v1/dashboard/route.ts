...
import { getUserScope, entryScopeFilter, workshopOrProjectScopeFilter, inventoryScopeFilterFor } from '@/lib/scope'
import { hasPermission, isPurchaseOverdue } from '@/lib/permissions'
...
export const GET = apiHandler(async ({ user }) => {
  ...
  // شاخهٔ «عملیاتی»: همهٔ نقش‌ها به‌جز مدیر پروژه/مدیر کل/حسابدار/ادمین (انباردار/کارپرداز/نیروی اجرایی/ناظر/بیننده لیستی می‌بینند)
  const isSupervisor = ['WORKSHOP_SUPERVISOR', 'WORKSHOP_MANAGER', 'WAREHOUSE_MANAGER', 'PURCHASER', 'FIELD_WORKER', 'INSPECTOR', 'VIEWER'].includes(user.role)
  // شاخهٔ «مدیرانه»: مدیر پروژه / مدیر کل / حسابدار / ادمین‌ها
  ...
  // ── وضعیت خرید و انبار و گزارش کار (سند سیستم یکپارچه) ──
  const canSeePurchases = hasPermission(user.role, 'purchase.view')
  ...
  const inventoryScope = (await inventoryScopeFilterFor(user, scope)) as Prisma.InventoryStockWhereInput
  const staleBefore = new Date(Date.now() - 24 * 60 * 60 * 1000)
  const [prPending, prApproved, prOrdered, prStale, lowStocks, totalStocks, workReportsToday] = await Promise.all([
    canSeePurchases ? db.purchaseRequest.count({ where: { AND: [purchaseScope, { status: 'PENDING' }] } }) : Promise.resolve(0),
    canSeePurchases ? db.purchaseRequest.count({ where: { AND: [purchaseScope, { status: 'APPROVED' }] } }) : Promise.resolve(0),
    canSeePurchases ? db.purchaseRequest.count({ where: { AND: [purchaseScope, { status: 'ORDERED' }] } }) : Promise.resolve(0),
    canSeePurchases ? db.purchaseRequest.count({ where: { AND: [purchaseScope, { status: 'PENDING' }, { createdAt: { lt: staleBefore } }] } }) : Promise.resolve(0),
    canSeeInventory
      ? db.inventoryStock.count({ where: { AND: [inventoryScope, { minQuantity: { not: null } }] } }).then(async (cands) => {
          // شمارش خطوطی که موجودی‌شان کمتر از حداقل است
          if (cands === 0) return 0
          const rows = await db.inventoryStock.findMany({
            where: { AND: [inventoryScope, { minQuantity: { not: null } }] },
            select: { quantity: true, minQuantity: true },
          })
          return rows.filter((r) => r.minQuantity != null && r.quantity < r.minQuantity).length
        })
      : Promise.resolve(0),
    canSeeInventory ? db.inventoryStock.count({ where: inventoryScope }) : Promise.resolve(0),
    db.workReport.count({ where: { AND: [workshopOrProjectScopeFilter(scope) as Prisma.WorkReportWhereInput, { reportDate: { gte: today } }] } }),
  ])

  // ── مالی ساده (سند سیستم یکپارچه — پرداخت/سررسید/بدهی) ──
  ...
  let finance: { debtTotal: string; overdueCount: number; overdueTotal: string; receivablesOpenAmount: string } | null = null
  if (canSeeFinance) {
    const debtPurchases = await db.purchaseRequest.findMany({
      where: { AND: [purchaseScope, { status: { in: ['ORDERED', 'RECEIVED'] } }, { totalAmount: { not: null } }] },
    ...
    for (const p of debtPurchases) {
      const paid = p.payments.reduce((a, x) => a + x.amount, 0n)
      const remaining = (p.totalAmount ?? 0n) - paid
      if (remaining > 0n) {
        debtTotal += remaining
        if (isPurchaseOverdue(p.dueDate, Number(remaining))) {
          overdueCount += 1
          overdueTotal += remaining
        }
      }
    }
    const openReceivables = await db.receivable.aggregate({
      where: { AND: [receivableScope, { status: 'OPEN' }] },
      _sum: { amount: true },
    })
    finance = {
      ...
      receivablesOpenAmount: (openReceivables._sum.amount ?? 0n).toString(),
    }
  }
  ...
  return ok({
    ...
    transfers,
    loans,
    purchases: { pending: prPending, approved: prApproved, ordered: prOrdered, stale: prStale },
    inventory: { lowStock: lowStocks, totalStocks },
    workReportsToday,
    finance,
