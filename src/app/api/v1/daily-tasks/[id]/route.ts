    ...
    return ok({
      task: {
        ...
        audio:
          task.sourceAudio
            ? {
                id: task.sourceAudio.id,
                fileName: task.sourceAudio.fileName,
                mimeType: task.sourceAudio.mimeType,
                size: task.sourceAudio.size,
                transcribeStatus: task.sourceAudio.transcribeStatus,
                audioUrl: makeSignedDailyAudioUrl(task.sourceAudio.id),
              }
            : null,
        comments: task.comments.map((c) => ({
          ...
          audio: c.audio
            ? { id: c.audio.id, fileName: c.audio.fileName, size: c.audio.size, audioUrl: makeSignedDailyAudioUrl(c.audio.id) }
            : null,
        })),
        canComplete,
        canEdit,
    ...
    await db.$transaction(async (tx) => {
      await tx.dailyTask.update({
        ...
        data: {
          ...
          sourceTranscript: body.sourceTranscript !== undefined ? body.sourceTranscript : undefined,
        },
      })
